...
|---|---|---|---|
| 30.07.2026 | 1.0 | Svetlana Fomina | Initial version, from the stakeholder interview |

## 2. Context and background

- **Problem or need:** athletes abandon a training plan after the first week they miss, because a
  static plan stops matching what they actually did.
- **Requested by:** the product owner, in the step 0 interview.
- **Why now:** the plan content can be drafted by an LLM provider, so adapting a plan per logged
  session is cheap enough to do on every write rather than in a weekly batch.
...
|---|---|---|
| The LLM provider degrades or raises latency | NFR-2 (8 s at p95) is missed and generation fails visibly at the moment a user first meets the product | p95 of the provider call drifts upward before timeouts start |
| In-process adaptation on write stops fitting the load | NFR-6 (60 s) is missed, and ADR-0001 has to be reopened with a queue | write latency grows with the number of concurrent logged sessions |
| Athletes log sessions irregularly | adaptation has too little signal, and the week-4 retention metric cannot be read | median sessions per active plan per week falls |
